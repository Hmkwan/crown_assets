...
Revision ID: patch_chat_attach_cols
...
revision = 'patch_chat_attach_cols'
...
def upgrade():
    ...
    if has_table('chat_attachment') and not has_column('chat_attachment', 'upload_user_id'):
    ...
    if has_table('chat_attachment') and has_column('chat_attachment', 'message_id'):
    ...
    if has_table('workflow_node') and not has_column('workflow_node', 'role_required_name'):
    ...
    if has_table('workflow_node') and not has_column('workflow_node', 'approver_user_id'):
        safe(op.add_column, 'workflow_node', sa.Column('approver_user_id', sa.Integer(), nullable=True))

    # workflow_node.approver_user_ids
    if has_table('workflow_node') and not has_column('workflow_node', 'approver_user_ids'):
        safe(op.add_column, 'workflow_node', sa.Column('approver_user_ids', sa.Text(), nullable=True))

    # approval_workflow.required_approvals
    if has_table('approval_workflow') and not has_column('approval_workflow', 'required_approvals'):
    ...
    if has_table('approval_workflow') and not has_column('approval_workflow', 'actions_on_reject'):
        safe(op.add_column, 'approval_workflow', sa.Column('actions_on_reject', sa.Text(), nullable=True))

    # workflow_template.is_default
    if has_table('workflow_template') and not has_column('workflow_template', 'is_default'):
