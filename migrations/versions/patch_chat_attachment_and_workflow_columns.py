...
Revision ID: patch_chat_attachment_and_workflow_columns
Revises: 
...
revision = 'patch_chat_attachment_and_workflow_columns'
...
def upgrade():
    ...
    def safe(f, *args, **kwargs):
        try:
            ...
            # Avoid failing migration on DBs that don't support the operation (e.g., SQLite ALTER nuances)
            print(f"Migration skip (non-fatal): {e}")

    # chat_attachment.upload_user_id
    if not has_column('chat_attachment', 'upload_user_id'):
        # Add as NULLABLE first; we'll backfill later then make NOT NULL in a follow-up migration
    ...
    if has_column('chat_attachment', 'message_id'):
        # Only alter if currently NOT NULL
...
def downgrade():
    ...
    # Be careful reverting nullable changes — skip changing message_id back to NOT NULL
