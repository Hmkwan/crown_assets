...
from app.models import User, Department, PartRequestOrder, WorkflowNode, ApprovalWorkflow
...
def test_workflownode_config_and_fallback():
    ...
    with app.app_context():
        ...
        # create a department and a requester
        ...
        # Ensure workflow nodes exist for part_request_order: department_head -> admin
        n1 = WorkflowNode(name='部门负责人审批', order_type='part_request_order', role_required='department_head', sequence=1)
        n2 = WorkflowNode(name='管理员审批', order_type='part_request_order', role_required='admin', sequence=2)
        ...
        steps = [(n.role_required, requester.department if getattr(n, 'department_specific', False) else None) for n in nodes]
...
def test_concurrent_approval_prevention():
    ...
    # second client attempts to approve the same approval; should not find a pending approval record for this user and return 404
