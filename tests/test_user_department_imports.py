...
def test_import_and_export_users():
    ...
    with app.app_context():
        ...
        resp = client.post('/admin/users/import', data=data, content_type='multipart/form-data', follow_redirects=True)
...
def test_import_and_export_departments():
    ...
    with app.app_context():
        ...
        resp = client.post('/admin/departments/import', data=data, content_type='multipart/form-data', follow_redirects=True)
        assert resp.status_code in (200, 302, 500)
        j = resp.get_json()
        if resp.is_json:
            assert j and j.get('success') is True
